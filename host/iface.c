...
usbInit(void){
	...
	if(!usbOpenDevice(&handle, USBDEV_SHARED_VENDOR, VNAME, USBDEV_SHARED_PRODUCT, PNAME)){
		printf("%s", "Cannot open device, exiting");
		return 1;
	}
...
void
usbData(void){
	recvBuffer = (char *) malloc(DEV_REQ_LEN);

	if( usb_control_msg(
        handle,
        USB_ENDPOINT_IN, // bRequestType
        0, // bRequest
        0, // wValue
        0, // wIndex
		...
		printf("%s", "Warning, recieved more/less data than expected");
}
